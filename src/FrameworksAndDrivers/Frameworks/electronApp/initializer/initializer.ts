...
// TODO MultitokenModePrint Check

ipcMain.on('beginMultiTokenPrint', (event: Event, selectedCategories: string[]) => {
  printPaper = new BrowserWindow({
    height: 250,
    width: 250,
    show: false,
  ...
  printPaper.loadFile('./views/printPaper.html');
  printPaper.webContents.on('did-finish-load', () => {
    printPaper.webContents.send('multiTokenNumber', selectedCategories);
  })
})

ipcMain.on('startPrinting', (e: Event, tokenString) => {
  console.log('Starting to print...' + tokenString);
